...
auth.onAuthStateChanged(user => {
  ...
  if (user) {
    ...
    welcomeBox.textContent = "Welcome! Assalaamu Alykum, " + user.displayName;
    welcomeBox.classList.add("show"); // fade in
    importOldDictionary();
    loadDictionary();
  } else {
    loginBtn.style.display = "inline";
    logoutBtn.style.display = "none";
    welcomeBox.classList.remove("show"); // fade out
    setTimeout(() => welcomeBox.textContent = "", 1000); // clear after fade
